...
def generate_knock_detector_model():
    ...
    model.compile(optimizer='adam', loss='binary_crossentropy', metrics=[modelMetrics])
...
# if shouldSave == 'y':
#     model.save('KnockPatternDetector1.tf')

model = load_knock_detector_model('KnockPatternDetector1.tf')
print(model.predict(['btfb !.mjae,gx',
                    'knock, knock!',
                    'hello, hello!',
                    'asdfg, asdfg!',
                    'ndiemldi, ndiemldi!']))